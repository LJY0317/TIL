...
4. package 정보를 파일로 만들기: ```$ pip freeze > requirements.txt```
5. .gitignore을 생성해두기: ```touch .gitignore``` 후 본문에 'venv' 작성
6. Django 프로젝트 생성: ```$ django-admin startproject firstpjt .```
7. 앱 생성 및 등록: ```$ python manage.py startapp articles``` 후 settings.py의 INSTALEED_APPS에 앱 이름 추가.
8. urls.py 분리: 앱 폴더에 'urls.py'를 생성 및 내용 작성.
9. 템플릿 상속을 위해 'base.html'을 정의.
  > 'base.html'이 있는 templates 폴더는 앱 폴더 외부에 있기 때문에 템플릿 경로를 추가로 등록해줘야 함.
10. 커스텀 유저 모델 미리 정의하고 설정하기:
  1) accounts 앱을 생성 및 등록
  2) urls.py 분리
  3) models.py에서 AbstractUser을 상속한 User 모델을 정의
  4) settings.py에서 AUTH_USER_MODEL의 값을 'accounts.User'로 수정
  5) migration 작업을 진행
  6) accounts/admin.py에서
...
4. base.html에 {% block content %}  {% endblock %} 뭐시기 해두자
  >
    ```python
    # 다른 html 파일에서 이렇게 상속 가능.
    {% extends 'base.html' %}
    {% block content %}
    {% endblock %}
    ```
1. settings.py의 TEMPLATES의 'DIRS': []에 BASE_DIR 등을 추가해보자.
...
25. 09. 30.
 ...
 6. 쟝고 프로젝트 생성
 7. 어플리케이션 생성 및 등록 (articles)
 8. urls.py를 분리 -> 복잡하니까 각 앱에서 관리하자
 9. 템플릿 상속을 위한 base.html을 정의
- base.html이 위치한 templates 폴더는 어플리케이션 폴더 외부에 있기 때문에 템플릿 경로를 추가로 등록을 해줘야만 함.
10. 커스텀 유저 모델 미리 정의하고 설정하기
- accounts 어플리케이션을 생성하고 등록한다.
- models.py에 가서 AbstractUser를 상속한 User 모델을 정의
- settings.py에 가서 AUTH_USER_MODEL의 값을 'accounts.User'로 수정
- migration 작업을 진행
- accounts/admin.py에서
